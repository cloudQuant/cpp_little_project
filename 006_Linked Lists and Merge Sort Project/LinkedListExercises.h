...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // -----------------------------------------------------------
  if (left.empty()){merged = right; return merged;}
  if (right.empty()){merged = left; return merged;}
  Node * left_cur = left.head_;
  Node * right_cur = right.head_;
  std::size_t num = left.size_ + right.size_;
  std::size_t i = 0;
  bool can_left_add = true;
  bool can_right_add = true;
  while (true){
    std::cout << " i = " << i << " now left = " << left_cur->data << " now right = " << right_cur->data << std::endl;
    if (left_cur == left.tail_ && right_cur == right.tail_){
        if (left_cur->data < right_cur->data){
          if (can_left_add){merged.pushBack(left_cur->data);can_left_add = false;}
          if (can_right_add){merged.pushBack(right_cur->data);can_right_add = false;}
    ...
    else if (left_cur == left.tail_ && right_cur != right.tail_){
        if (left_cur->data > right_cur->data){
            ...
            if (can_left_add){merged.pushBack(left_cur->data);can_left_add = false;}
            else{merged.pushBack(right_cur->data);right_cur = right_cur->next;}
        }
        ...
        if (left_cur->data > right_cur->data){
          if (can_right_add){merged.pushBack(right_cur->data);can_right_add = false;}
          else {merged.pushBack(left_cur->data);left_cur = left_cur->next;}
        }
        else{
    ...
    if (!can_left_add && !can_right_add){std::cout << "ends============" <<std::endl; break; }

    i++;
    if (i > num) {std::cout << "ends============" <<std::endl;break;}
  }
