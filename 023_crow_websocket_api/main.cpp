...
void getView(response &res, const string &filename, const context & x) {
...
void sendFile(response &res, const std::string & filename, string contentType){

    std::string file_name = "public/" + filename;

    if (std::filesystem::exists(file_name)) {
        std::cout << "File exists!" << std::endl;
    } else {
        std::cout << file_name << "File does not exist!" << std::endl;
    }
  ...
  if(in){
    ...
    contents << in.rdbuf();
    in.close();
    res.set_header("Content-Type", std::move(contentType));
...
void sendHtml(response &res, const std::string & filename){
  sendFile(res, filename + ".html", "text/html");
}

void sendImage(response &res, const std::string &filename){
...
// Check if the collection exists, and drop it if it does
    ...
    if (std::ranges::find(collection_names, "contracts") != collection_names.end()) {
        db["contracts"].drop();
    }
  ...
  CROW_ROUTE(app, "/styles/<string>")
    ([](const request &req, response &res, const std::string & filename){
  ...
  CROW_ROUTE(app, "/about")
   ...
   ([](const request &req, response &res, const std::string & a, const std::string & b){
