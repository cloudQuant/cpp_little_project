...
void print_s(int distance[]) {
  for (int i = 0; i < 8; i++) {
    std::cout << "s to value " << i
              << " is " << distance[i] << std::endl;
  }
}
void DisjointSets::bfs(int i, int n, int m, int edges[][2]) {
  ...
  for (int d = 1; d <= m; d++) {
    ...
    // iterations.
    int rooti = find(i);
    //std::cout << " i = " << i << " rooti = " << rooti <<std::endl;
    ...
    for (int j = 0; j < m; j++) {
      ...
      if ( root0 == rooti && root1 != root0) {
        ...
        //std::cout << "root0 = " << root0 << " root1 = " << root1 << " rooti = " << rooti << " f = " << f << std::endl;
        //print_s(s);
        //print_distance(distance);

      }
      else if ( root1 == rooti && root1 != root0) {
