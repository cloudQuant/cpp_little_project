...
void deal_command(std::vector<std::string> &vec, std::set<std::string>&con_set, std::string &cons){
    ...
    while (!wordStack.empty()) {
        ...
        if (con_set.count(word)){
            if (word=="copy"){
                int N = my_string_to_i(newWordStack.top());
                newWordStack.pop();
                int X = my_string_to_i(newWordStack.top());
                newWordStack.pop();
                int L = my_string_to_i(newWordStack.top());
                newWordStack.pop();
                ...
                if (vec[N-1].rfind(s)!=std::string::npos){
                    std::size_t pos = vec[N-1].rfind(s);
                    // 将整数pos转换为字符串
                    std::stringstream ss;
                    ss << pos;
                    ss >> index;
...
int main(){
    ...
    while (std::getline(std::cin, con)){
        if (con == "over"){
            ...
            my_cons.push_back(con);
        }
    }
    for (int i =0; i<my_cons.size(); i++){
        std::string my_con = my_cons[i];
        deal_command(vec, con_set, my_con);
    }
    return 0;
}
